...
type Props = FieldHookConfig<string> & {
  ...
  isTouched?: boolean;
  disabled?: boolean;
  getCountryPhoneObject: (obj: any) => void;
};

export const FormPhoneInputWithValidation: React.FC<Props> = ({
  label,
  className,
  help,
  type = "text",
  ...
  isTouched = false,
  disabled = false,
  getCountryPhoneObject,
  ...props
}) => {
  const [field, meta, helpers] = useField(props);
  const id = nanoid();

  const handleChange = (param, country) => {
    getCountryPhoneObject && getCountryPhoneObject(country);
    const value = param.startsWith('+') ? param : '+' + param;
...
const Container = styled.div<ContainerProps>`
  ...
  .flag-dropdown,
  input {
    border: 1px solid ${(props) => props.status === "error"
      ? "red"
      : props.status === "active"
        ? "rgba(0, 0, 0, 0.8)"
